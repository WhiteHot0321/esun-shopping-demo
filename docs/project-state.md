...
Updated: 2026-09-25 (Phase 3.2 #16 CI/CD and #17 API docs closed; strategy: engineering depth)
...
- **Phase 3.2 #17 — API 文件 [Small], complete and verified (self-review only) — 2026-09-25, Claude Code, branch `feature/phase32-17-api-docs`.** springdoc-openapi 2.6.0 generates the OpenAPI 3 spec (`/v3/api-docs`, `.yaml`) and Swagger UI (`/swagger-ui.html`) from the 12 controllers (`@Tag` per controller, `@Operation(summary)` on all 51 paths, roles named in the summary). `JwtAuthFilter.isPublicRoute` is now a public static used by both the filter and `OpenApiConfig`, so the spec's bearer-auth marks cannot drift from what the filter enforces (a test compares every operation); docs paths are GET-only public routes. `API_DOCS_ENABLED` (default true, **set false in production**) removes the endpoints (404, tested). Verification: backend `mvn test` **246/246** (+6), JaCoCo PASS; live app: api-docs 200 (51 paths, 12 tags), UI 200, `/api/orders` still 401. Known follow-ups: field-level `@Schema`, error examples, deployment default of `API_DOCS_ENABLED=false`, not independently reviewed. Details: `docs/tasks/040-api-docs.md`, `docs/handoff/phase32-17-api-docs.md`. Engineering note: derive documentation from the enforcement code (one policy function) instead of writing a second list, and test the two against each other.
- **Phase 3.2 #16 — CI/CD 自動化 [Maintainer / Small-Medium], implemented and merged into `advanced-v2` on 2026-09-25; GitHub-side run NOT yet observed — 2026-09-24, Claude Code, branch `feature/phase32-16-cicd` (no PR: no GitHub CLI on this machine).** Rewrote `.github/workflows/ci.yml`: push only on main/advanced-v2/`v*` tags plus `pull_request` and `workflow_dispatch`; concurrency cancels superseded runs except on protected refs; global `contents: read`; surefire + JaCoCo reports uploaded with `if: always()`; buildx + GHA cache image build without push for PRs; new `publish-image` job pushes `ghcr.io/<repo>/backend` with the built-in `GITHUB_TOKEN` (only job with `packages: write`, only after backend-test + frontend-build + docker-build are green, never on PRs). Added `.github/dependabot.yml` (maven/npm/docker/actions), `backend/.dockerignore`, README CI badge. No application code touched. Verification: workflow YAML parsed and job graph/permissions inspected; local `docker build` of the backend image PASS (~2 min). Not proven: a real Actions run, the first GHCR push (only happens after merge into advanced-v2), branch protection/required checks, image scan/SBOM. Deliberately no deploy job (target undecided), frontend image not built (no Dockerfile), k6 suite not wired into CI; not an independent or Codex review. Details: `docs/tasks/039-cicd-automation.md`, `docs/handoff/phase32-16-cicd.md`. Engineering note: PRs (incl. forks) get credential-free verification only; registry write permission is granted to one job on protected refs.
