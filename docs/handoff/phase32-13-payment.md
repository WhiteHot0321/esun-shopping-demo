...
Risks: sandbox 預設關閉——本機需 `PAYMENT_SANDBOX_ENABLED=true` 才能付款；賣家仍可對未付款訂單出貨（業務決策待定）。

Next: 收藏/心願單、優惠券（019 Phase 3.2）或 Phase 3 工程深度主線；若要上真實 provider，實作 `PaymentGateway` 並取得測試帳號。
