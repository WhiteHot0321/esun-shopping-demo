...
Live verified: ECPay stage 接受我方簽章表單、拒絕重送同一 MerchantTradeNo（10300028）；瀏覽器 E2E（失敗→重付→已付款→取消→待退款）。Not proven: ECPay 真實入站回調（需公開 HTTPS callback URL）；Codex 獨立審查；藍新；退款流程、逾期自動取消、付款稽核、callback 速率限制。
