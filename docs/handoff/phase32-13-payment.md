...
Validated: backend `mvn clean test` **206/206**（含 ECPay 整合）, JaCoCo PASS; frontend Vitest **58/58**, checkout 3/3, build PASS. 獨立（全新脈絡、唯讀）審查 PASS，3 項 should-fix 已修復並重測。
...
Risks: 未合併分支 `codex/phase31-pay-status` 含另一版真實 ECPay 實作，類別名衝突、表不同，勿直接 merge，應把其 `EcpayPaymentGateway` 移植為 `PaymentGateway` 的第二實作（見 task 035）；預設 `payment.provider=none`——本機需 `PAYMENT_PROVIDER=sandbox`（或 ecpay + ecpay.* 設定）才能付款；ECPay stage 端對端尚未實測；賣家仍可對未付款訂單出貨（業務決策待定）。
